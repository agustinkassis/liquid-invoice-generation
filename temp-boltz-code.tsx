// app/api/boltz/reverse/route.ts
...
function parseLiquidDescriptors(envStr: string): ParsedDescriptors {
...
function deriveLiquidAddress(xpub: string, index: number, masterBlindingKeyHex: string) {
...
async function createBoltzReverseSwap(amountMsat: number) {
  ...
  return (await res.json()) as { id: string; invoice: string }
}
...
export async function GET(req: NextRequest) {
  try {
    ...
    if (!Number.isFinite(amountMsat)) {
      return NextResponse.json({ error: "Invalid amountMsat" }, { status: 400 })
    }

    const parsed = parseLiquidDescriptors(process.env.LIQUID_DESCRIPTORS || "")
    const index = inMemoryExternalIndex++
